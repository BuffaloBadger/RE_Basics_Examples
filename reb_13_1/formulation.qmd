...
This assignment describes a single BSTR and its operation. There are no other reactors, heat exchangers, stream splits or stream mixers. The reaction taking place in the reactor is identified, and a rate expression is provided for it. The quantities of interest in the assignment are reactor outputs, as defined in @sec-3_design_eqns. This combination of characteristics identifies this as an isolated reactor modeling assignment.

I know the general approach to completing an isolated reactor modeling assignment, and I will implement it here, but first I will summarize the information provided in the assignment.
...
**Responses of Interest**: $C_A\left(t\right)$, $C_B\left(t\right)$, $C_Y\left(t\right)$, $C_Z\left(t\right)$, and $T\left(t\right)$.
...
At this point, there are no missing reactor inputs, properties or operating parameters, and the reactor design equations can be solved. The next step in the general approach to completing an isolated reactor modeling assignment is to write the ancillary equations needed to calculate the response(s) of interest.

Solving the design equations will give values of $n_A$, $n_B$, $n_Y$, $n_Z$, and $T$ at times between $t=0$ and $t = t_{\text{rxn}}$. The responses of interest are the concentrations and temperature *vs*. $t$. The concentrations can again be calculated using the defining equation for concentration.
...
The next step in the general approach to completing an isolated reactor modeling assignment is to create a response function. In this assignment, there were no missing reactor inputs, reactor properties, or operating parameters that need to be passed to the response function. It simply needs to solve the design equations and compute the responses of interest as described above.
...
The final step in the general approach to completing an isolated reactor modeling assignment is to use the response function to calculate the unknown quantities of interest. In this assignment, the unknown quantities of interest **are** the responses, so they can be calculated simply by executing the response function.
...
2. Plot each of the concentrations, the temperature, and the rate versus time.
...
In fact, the curvature does change because at some point the effect of decreasing reactant concentrations upon the rate becomes stronger than the effect of increasing temperature. At the point where the two effects becom equal, the rate reaches a maximum. Beyond the maximum, the effect of decreasing reactant concentration dominates and the rate decreases. This is seen in @fig-example_13_1_rates.
