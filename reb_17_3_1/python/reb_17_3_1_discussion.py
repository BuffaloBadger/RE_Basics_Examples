...
def perform_the_analysis():
    # set initial guess for the unknowns for a zero conversion steady state
    nDotA_0 = Vdot_0*CA_0
    initial_guess = np.array([2.3, 0, T_0])
    ...
    Vdot_3 = Vdot_0
    CA_3_zero = nDotA_3/Vdot_3*1000
    CZ_3_zero = nDotZ_3/Vdot_3*1000

    # set initial guess for the unknowns for a low conversion steady state
    nDotA_0 = Vdot_0*CA_0
    initial_guess = np.array([1.5*nDotA_0, 0.5*nDotA_0, T_0 + 5])
    ...
    # calculate the product concentrations
    Vdot_3 = Vdot_0
    CA_3_low = nDotA_3/Vdot_3*1000
    CZ_3_low = nDotZ_3/Vdot_3*1000
    
    # tabulate the results
    data = [['CA 3 zero',CA_3_zero,'M']
            ,['CZ 3 zero',CZ_3_zero,'M']
            ,['T 3 zero',T_3_zero,'K']
            ,['CA 3 low',CA_3_low,'M']
            ,['CZ 3 low',CZ_3_low,'M']
            ,['T 3 low',T_3_low,'K']]
    ...
    old_df = pd.read_csv('reb_17_3_1/python/results.csv')

    # append the new results to the dataframe
    new_df = old_df.append(results_df, ignore_index=True)
    ...
    # save the results
    new_df.to_csv('reb_17_3_1/python/results.csv', index=False)
