...
df = pd.read_csv('./reb_8_2/reb_8_2_data.csv')
...
for T_K in block_temperatures:
    ...
    plt.tight_layout()
    filename = filepath + 'reb_8_2_model_T_equals_' + T_as_text + '.png'
...
result.to_csv(filepath + "reb_8_2_Arrhenius_results.csv", index=False)
...
plt.xticks(rotation=25)
plt.tight_layout()
plt.savefig(filepath + 'reb_8_2_Arrhenius_plot.png')
plt.show()
